...
const sbtcTransferManyNativeExtension =
  deployments.extSponsoredSbtcTransferManyNative.simnet;

describe("Sponsored sBTC Transfer Many Native Extension", () => {
  it("non-owner cannot call the sBTC transfer many native extension", () => {
    ...
    const { result: sbtcTransferManyResult } = simnet.callPublicFn(
      ...
      [
        ...
        Cl.principal(sbtcTransferManyNativeExtension),
        // (payload (buff 2048))
        Cl.bufferFromHex(
          Cl.serialize(
            Cl.tuple({
  ...
  it("sending sBTC to one recipient using transfer many native extension correctly updates balances", () => {
  ...
  it("sending sBTC to one recipient using transfer many native extension correctly prints the events", () => {
    const fundingAmount = 100;
    const transferAmount = 50;
    const feesAmount = 1;
    ...
    expect(payloadData).toEqual({
      a: "extension-call",
      payload: {
        extension: sbtcTransferManyNativeExtension,
  ...
  it("sending sBTC to multiple recipients using transfer many native extension correctly updates balances", () => {
    const fundingAmount = 100;
  ...
  it("sending sBTC to multiple recipients using transfer many native extension correctly prints the events", () => {
    const fundingAmount = 100;
    const transferAmount1 = 10;
    const transferAmount2 = 20;
    const transferAmount3 = 30;
  ...
  it("sending sBTC to N recipients using transfer many native extension correctly updates balances", () => {
    ...
    const N = 18; // 18 is the maximum number of standard principal recipients.
